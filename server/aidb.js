...
exports.init = function (conf, logger) {
  ...
  db = new Db('aidbjs', server, {safe: true});
  db.open( function (err, db) {
    ...
    db.createCollection('aidbjs', function (err, coll) {
      if (err) {
        logger.error(err);
        throw err;
      }
      db.ensureIndex('aidbjs', {hash: 1}, {unique: true, dropDups: true}, function (err, name) {
...
exports.strongVerify = function (req, res, next) {
  var hash = req.params.hash;  // needs restify.queryParser plugin 
  var hash2 = req.params.hash2;  // needs restify.queryParser plugin 
  ...
  if (!hash2)
    return next(new restify.InvalidArgumentError("Missing parameter 'hash2'"));

  req.log.debug('hash sequence to verify: ' + algorithm + ':' + hash + ':' + hash2);
  db.collection('aidbjs', function(err, collection) {
...
exports.sign = function (req, res, next) {
  ...
  if (!owner)
    return next(new restify.InvalidArgumentError("Missing parameter 'owner'"));

  if (!seed)
    return next(new restify.InvalidArgumentError("Missing parameter 'seed'"));
